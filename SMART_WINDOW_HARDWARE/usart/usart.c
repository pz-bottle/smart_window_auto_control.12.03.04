...
u8 gsm_rxbuf[GSM_RXBUF_SIZE];
volatile u8 gsm_index;
...
void USART3_Init(u32 bound)
{
  ...
  GPIO_InitValue.GPIO_Mode = GPIO_Mode_IPD;
...
void USART2_IRQHandler(void)
{
	...
	if(RESET != USART_GetITStatus(USART2,USART_IT_RXNE))
	{
		...
		gsm_rxbuf[gsm_index] = (u8)USART2->DR;
...
{
	while (USART_GetFlagStatus(PRINTF_COM, USART_FLAG_TC) == RESET) {}//先判断，否则会丢失第一个字符	
